        ...
        while True:
            ...
            if masks is not None:
                ...
                for seg, box in zip(masks.data.cpu().numpy(), boxes):
                    ...
                    dist = ''
                    dist = dist + str(distance_up) + '/'
                    dist = dist + str(distance_diag_left) + '/'
                    dist = dist + str(distance_diag_right)
                    # dist = [distance_up, distance_diag_left, distance_diag_right]
                    # data = input("전송할 데이터를 입력하세요: ")  # 또는 센서 등에서 데이터를 읽어올 수 있음
                    conn.sendall(dist.encode())
